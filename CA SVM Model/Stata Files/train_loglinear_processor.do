...
/*
	THIS ONE HELPS A LOT, the other tweaks only improved performance slightly.
...
foreach i of local small {
	drop if (opt_du>`i' & lot_size_sqft < 950*`i') 
}
*/

// Remove 80% of observations with negative labels
gen rnd = runiform() if bp_exists==0
replace rnd=0 if bp_exists==1
drop if rnd > 0.07
drop rnd
 
// Triple the number of observations with positive labels
gen rnd = runiform() if bp_exists==1
replace rnd=0 if bp_exists==0
expand 2 if rnd>0
drop rnd
...
save "train_loglinear_97", replace
